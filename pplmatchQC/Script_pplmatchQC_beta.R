...
#' columns: `speaker_full_name` (speaker's name) and `event_date` (date of the debate).
...
pplmatchQC <- function(Corpus, Names, max_dist = 0.15) {
  ...
  Corpus_with_leg <- Corpus %>%
    ...
    mutate(
      ...
      speaker_clean = str_to_lower(str_replace_all(speaker_full_name, "[^[:alpha:] ]", ""))
    )
  ...
  Speakers <- Corpus_with_leg %>%
    distinct(speaker_full_name, speaker_clean, legislature_id)
  ...
  Speaker_matches <- stringdist_inner_join(
    ...
    group_by(speaker_full_name, legislature_id) %>%
    slice_min(.dist, n = 1, with_ties = FALSE) %>%
    ungroup() %>%
    select(speaker_full_name, legislature_id, matched_name = full_name, party_id, gender, name_source)
  
  # Step 5: Merge with the original corpus
  Corpus_matched <- Corpus_with_leg %>%
    left_join(Speaker_matches, by = c("speaker_full_name", "legislature_id"))
